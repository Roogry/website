    ...
    <div class="container pt-5">
        <form action="" method="POST">
            ...
            ?>
                ...
                <div class="row mt-2">
                    <a class="btn btn-light col-md-5 m-2 ml-auto" href="?count=<?=++$count?>">Tambah Inventaris</a>
                    <a class="btn btn-light col-md-5 m-2 mr-auto" href="?count=<?=$count - 2?>">Hapus Inventaris</a>
                </div>
...
<?php
    if(isset($_POST ['pinjam'])){
        ...
        $cekPinjaman = $conn->query("SELECT * FROM peminjaman WHERE id_pegawai = '$id_peminjam' AND status_peminjaman = '1'");
        ...
        if($cekPinjaman->rowCount() > 0){
            ...
            $kode = "PN".$string;
            $setPinjam = $conn->query("INSERT INTO peminjaman (id_pegawai, kode_peminjaman, tanggal_kembali) VALUES ('$id_peminjam', '$kode', '$tgl')");

            $idPeminjaman = $conn->lastInsertId();

            for($i = 0; $i < sizeof($invent); $i++){
                $setDetailPinjam = $conn->query("INSERT INTO detail_pinjam (id_inventaris, id_peminjaman, jumlah) VALUES ('$invent[$i]', '$idPeminjaman', '$jumlah[$i]')");
            }
